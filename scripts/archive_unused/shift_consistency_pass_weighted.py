...
def main():
    ...
    ap.add_argument('--out-prefix',default=r'.\CONSIST')
    ...
    need=['gear_actual','Time of Latest Shift','throttle_pct','speed_mph','__file']; frames=[]
    ...
    if not frames: print('[CONSIST_W] No data'); return
    d=pd.concat(frames,ignore_index=True)
    for c in ['speed_mph','throttle_pct','gear_actual','Time of Latest Shift']: d[c]=pd.to_numeric(d[c],errors='coerce')
    d=d.dropna(subset=['gear_actual'])
    ...
    for g in [2,3,4,5]:
        gd=d[d['gear_actual']==g]; ts=gd['Time of Latest Shift'].dropna()
        if ts.empty: continue
        interval=ts.diff().dropna()
        if interval.empty: continue
        var=float(interval.var()); wmed=float(gd['w'].median()) if not gd['w'].empty else 1.0
        score=var*wmed
        if score>5.0:
            for t in [25,31,37,44,50]:
                i=tps_idx(t); up[f'{g} -> {g+1} Shift'][i]=(+0.2) if math.isnan(up[f'{g} -> {g+1} Shift'][i]) else (up[f'{g} -> {g+1} Shift'][i]+0.2)
                dn[f'{g+1} -> {g} Shift'][i]=(-0.2) if math.isnan(dn[f'{g+1} -> {g} Shift'][i]) else (dn[f'{g+1} -> {g} Shift'][i]-0.2)
    out=args.out_prefix.rstrip('\\/'); write_delta(f'{out}__SHIFT_UP__DELTA.tsv',up); write_delta(f'{out}__SHIFT_DOWN__DELTA.tsv',dn); print('[CONSIST_W] WROTE',f'{out}__SHIFT_UP__DELTA.tsv',f'{out}__SHIFT_DOWN__DELTA.tsv')
