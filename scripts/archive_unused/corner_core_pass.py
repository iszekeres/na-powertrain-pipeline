...
def main():
    ...
    ap.add_argument("--thr-rate",  type=float, default=16.0)  # pct/sec
    ...
    for fp in files:
        ...
        dthr = np.gradient(thr, t, edge_order=1)
        # downshifts while aggressive tip-in inside speed window
        dn = np.where(np.diff(ga)==-1)[0]
        for i in dn:
            if not (mspd[i] and dthr[i]>=args.thr_rate): continue
    ...
    p = write_delta(args.out_dir, "CORNER_CORE__SHIFT_DOWN__DELTA.tsv", ROWS_DN, deltas)
    print(f"[OK] CORNER CORE → {p} | nonzero_cells={int((deltas!=0).sum())}")
