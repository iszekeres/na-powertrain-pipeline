...
# -*- coding: utf-8 -*-
"""
trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE.py — baked-in normalization (requires renaming your current overlay to: trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE__BASE.py)
Workflow:
  1) This file launches trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE__BASE.py with all the same CLI args.
...
Why this design?
  - You asked to "bake into the original scripts." Replacing your original file
    ...
    overlay logic lives in a sibling file named trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE__BASE.py.

One-time step:
  Rename your current script to trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE__BASE.py in the same folder as this file.

Run exactly like before (same flags).

Example:
  python .\trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE.py --tag MULTILOG_RAW_WEIGHTED --review-dir ".\06_Logs\Trans_Review" --gap 6 [...]
"""
...
LABEL_SUFFIX = "MAXATTACK"
INNER = Path("trans_overlay_ultra_street_racer_MAXATTACK__v2_TABSAFE__BASE.py")
...
def normalize_up_down(review_dir: Path, tag: str, gap: float):
    up_p = review_dir / f"HPT_paste__16col__UPSHIFT__{tag}__MAXATTACK.tsv"
    dn_p = review_dir / f"HPT_paste__16col__DOWNSHIFT__{tag}__MAXATTACK.tsv"
