...
# -*- coding: utf-8 -*-
"""
Ultra Plush Overlay (v2.1-hardened) — NA Powertrain Tuning
- 16-col TPS axis; monotonic rows; ~gap mph hysteresis
- SMART SIXTH (plush): 5->6 floors (~58..84 mph thru 69% TPS; 200 at ≥81%), no try/except
...
- TCC Release = Apply − 3
- WOT blocks include up + down (whole-mph; uses project caps)
- Slip 9×9 per gear: clamp 5–30 rpm; integers; non-increasing down torque rows
"""
import argparse
import numpy as np
import pandas as pd
...
def write_tsv(df: pd.DataFrame, p: Path) -> None:
    ...
    # row-wise forward fill for NaNs (no sentinels)
...
# --- Shift overlays (light touch, enforce monotonic) ---
...
def write_slip_9x9(tag, out_dir, vmin, vmax, base_min, base_max, g_offsets):
    ...
    for gear, off in g_offsets.items():
        ...
        with open(Path(out_dir) / out_name, "w", encoding="utf-8") as f:
            for r in range(rows):
...
def main():
    ...
    # SMART SIXTH (plush) — enforced, no silent skip
    ...
    # Harden 6->5 to track under 5->6 by ~gap (slightly more eager ≥75% TPS)
    ...
    if r56 is not None and "6->5" in labels_dn:
        ...
        for c, uv in zip(cols, up_vals):
            if uv >= 300:  # unlocked bins unaffected
                dn_vals.append(pd.to_numeric(dn_out.at[r65, c], errors="coerce"))
                continue
            tps = int(float(c))
            eager = 1 if tps >= 75 else 0
            target = max(1.0, uv - args.gap - eager)
            cur = pd.to_numeric(dn_out.at[r65, c], errors="coerce")
            curf = float(cur) if not pd.isna(cur) else target
    ...
    print(f"[OK] ULTRA_PLUSH v2.1-hardened written to {rd} (UP/DOWN/TCC/WOT + slip 9x9, smart-sixth, clamp [5,30])")
